...
public class POEExtraction implements Indication, SubIndication, NodeName, NodeValue, AttributeName, AttributeValue, ExceptionMessage {
    ...
    public void addPOE(final XmlDom timestamp, final XmlDom certPool) {
        ...
        if (date == null) {
            ...
            throw new DSSException(EXCEPTION_TPTCNBN);
        }
        LOG.fine("Extraction of POE from the timestamp at: " + date);
            ...
            if (SIGNED_SIGNATURE.equals(nodeName)) {
                ...
                if (CERTIFICATE.toUpperCase().equals(category)) {
                    ...
                    try {
                        ...
                        Integer certificateId = certPool
                              .getIntValue("./dss:Certificate/dss:DigestAlgAndValue[dss:DigestValue='%s']/../@Id", digestValue);
                        addCertificatePoe(certificateId, date);
                    } catch (Exception e) {

                        LOG.severe(String.format("The certificate with digest value:%S is not found.", digestValue));
