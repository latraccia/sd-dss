...
public class PdfPKCS7 {
   ...
   private Collection<Object> certs, crls, signCerts;
   ...
   static {
      ...
      algorithmNames.put("0.4.0.127.0.7.1.1.4.1.6", "ECDSA");
   ...
   public PdfPKCS7(byte[] contentsKey, String provider) {
      try {
         ...
         if (!objId.getId().equals(ID_PKCS7_SIGNED_DATA)) throw new IllegalArgumentException("Not a valid PKCS#7 object - not signed data");
         ...
         // ORIGINAL: DEROctetString rsaDataContent =
         ...
         if (signerInfos.size() != 1) throw new IllegalArgumentException("This PKCS#7 object has multiple SignerInfos - only one is supported at this time");
         ...
         for (Iterator<Object> i = certs.iterator(); i.hasNext();) {
         ...
         if (signerInfo.getObjectAt(next) instanceof ASN1TaggedObject) {
            ...
            if (digestAttr == null) throw new IllegalArgumentException("Authenticated attribute is missing the digest.");
            ++next;
         }
    ...
    */
   public PdfPKCS7(PrivateKey privKey, Certificate[] certChain, CRL[] crlList, String hashAlgorithm, String provider, boolean hasRSAdata) throws InvalidKeyException, NoSuchProviderException,
            NoSuchAlgorithmException {
      this.privKey = privKey;
      this.provider = provider;

      digestAlgorithm = allowedDigests.get(hashAlgorithm.toUpperCase());
      if (digestAlgorithm == null) throw new NoSuchAlgorithmException("Unknown Hash Algorithm " + hashAlgorithm);
   ...
   /**
    * Get all the X.509 certificates associated with this PKCS#7 object in no particular order. Other certificates, from
    * OCSP for example, will also be included.
    * 
   ...
   private void signCertificateChain() {
      ...
      cc.add(signCert);
      ArrayList<Object> oc = new ArrayList<Object>(certs);
    ...
    */
   public Collection<Object> getCRLs() {
      return crls;
   }
    ...
    * @param crls the certificate revocation list or <CODE>null</CODE>
    * @param calendar the date or <CODE>null</CODE> for the current date
    * @return <CODE>null</CODE> if the certificate chain could be validated or a <CODE>Object[]{cert,error}</CODE> where
    *         <CODE>cert</CODE> is the failed certificate and <CODE>error</CODE> is the error message
    */
    ...
    *           <CODE>digest</CODE> is also <CODE>null</CODE>. If the <CODE>digest</CODE> is not <CODE>null</CODE> then
    *           it may be "RSA" or "DSA"
    */
   ...
   public byte[] getEncodedPKCS7(byte secondDigest[], Calendar signingTime, TSAClient tsaClient, byte[] ocsp) {
      try {
         ...
         if (!crls.isEmpty()) {
            v = new ASN1EncodableVector();
            for (Iterator<Object> i = crls.iterator(); i.hasNext();) {
   ...
   /**
    * Added by Aiken Sam, 2006-11-15, modifed by Martin Brunecky 07/12/2007 to start with the timeStampToken (signedData
    * 1.2.840.113549.1.7.2). Token is the TSA response without response status, which is usually handled by the (vendor
    ...
    * that encoding and the parameters must be exactly the same as in {@link #getEncodedPKCS7(byte[],Calendar)}.
    * <p>
    * A simple example:
    * <p>
    * 
